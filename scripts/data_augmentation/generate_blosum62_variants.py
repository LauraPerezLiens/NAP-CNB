...
# Precompute the most likely amino acid substitution for each standard amino acid.
BEST_CHANGE = {}
...
def most_probable_change(aa: str) -> str:
    """Return the most conservative BLOSUM62-based substitution for one amino acid."""
...
def build_variants(seq: str) -> List[str]:
    ...
    The returned list contains:
        ...
        2. One mutated sequence per position.

    Each mutated sequence differs from the original by exactly one amino acid.
    """
...
def flush_buffer(
    ...
    """Write buffered rows to disk and return updated header status and row count."""
...
def main() -> None:
    ...
    for group_id, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        # group_id links the original sequence with all its BLOSUM-derived variants.
        ...
        if group_id % CHUNK_SIZE == 0:
            logging.info("Processed input rows: %d", group_id)
    ...
    logging.info("Groups generated: %d", total_groups)
